...
responsiShortcodeMeta={
	...
	customMakeShortcode: function(a){
		...
		jQuery( ".sort_table .sort_table_item").each(function (index) {
			...
			if( arr.unordered_list == 'yes'){
				delete arr.unordered_list;
				delete arr.ordered_list;
				var f = "";
				for (var i in arr) {
					if( i != 'content' ){
						if(i != 'style' ){
		        ...
		        if( f != '' ){
		        	shortcode += '[a3_unordered_list '+ f +']<ul>';
					shortcode += "<li>" + "Place your list items here" + "</li>"
					shortcode += '</ul>[/a3_unordered_list]';
		        }
			}

			if( arr.ordered_list == 'yes'){
		        shortcode += '[a3_ordered_list style="'+ arr.style +'"]<ol>';
				shortcode += "<li>" + "Place your list items here" + "</li>"
				shortcode += '</ol>[/a3_ordered_list]';
			}
        });
		return shortcode;
	},
