...
class Lists {
	...
	public function __construct () {
        ...
        add_shortcode('a3_unordered_list', array($this, 'render_unordered_list'));
		add_shortcode('a3_ordered_list', array($this, 'render_ordered_list'));
	}

	public static function _shortcode_hook( $responsi_list_shortcode ){
		$responsi_list_shortcode[] = 'a3_unordered_list';
		$responsi_list_shortcode[] = 'a3_ordered_list';
		return $responsi_list_shortcode;
	}

	public function render_ordered_list($atts, $content = null) {
	...
	public function render_unordered_list($atts, $content = null) {
		do_action('_front_enqueue_css_and_js');
		global $responsi_options_a3_shortcode;
		if( is_array($responsi_options_a3_shortcode)){
			$default_options = $responsi_options_a3_shortcode;
		...
		if( $fronticon != 'yes' && $frontimage == 'yes' && $image && $image_width && $image_height ) {
			...
			$icons = sprintf( '<i class="listicon"><img src="%s" width="%s" height="%s" alt="%s" class="image-icon%s" %s %s /></i>', $image, $image_width, $image_height, $alt, $class, $data_animation, $css  );
		}

		if( $fronticon == 'yes' && $frontimage != 'yes' && $icon != '') {
			$icons = sprintf( '<i class="listicon responsi-shortcode-icon shortcode-icon-%s%s%s" %s %s></i>', $icon, $size_class,$class, $data_animation, $css );
		}

		//return $content;

		if ( $icons != '' && $fronticon == 'yes' && isset( $icon ) && $icon != '' ){
			$content = str_replace('<li>','<li>'.$icons.'<span class="listcontent">',$content);
			$content = str_replace('</li>','</span></li>',$content);
			return '<div class="shortcode-unorderedlist shortcode-unorderedlisticon">' . do_shortcode( $content ) . '</div>' . "\n";
		}elseif( $icons != '' && $frontimage == 'yes' && $image && $image_width && $image_height ){
			$content = str_replace('<li>','<li>'.$icons.'<span class="listcontent">',$content);
			$content = str_replace('</li>','</span></li>',$content);
			return '<div class="shortcode-unorderedlist shortcode-unorderedlisticon shortcode-unorderedlistimage">' . do_shortcode( $content ) . '</div>' . "\n";
		}
	}
